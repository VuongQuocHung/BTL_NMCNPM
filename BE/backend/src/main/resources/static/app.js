const App = (() => {
  ...
  function updateHeader() {
    ...
    if (accountLink && user) {
      ...
      accountLink.href = isAdmin ? "admin.html" : "profile.html";
    }
  ...
  function productCard(product) {
    ...
    return `
      <article class="card">
        ...
        <div>
          <h3><a href="product.html?id=${product.id}">${html(product.name || "Không tên")}</a></h3>
  ...
  async function initHome() {
    ...
    $("#homeCategories").innerHTML = pageContent(categories).map((category) => `
      <a class="card" href="products.html?categoryId=${category.id}">
  ...
  async function initProductDetail() {
    ...
    $("#productDetail").innerHTML = `
      ...
      <div class="detail-info">
        ...
        <div class="actions">
          <button class="button primary" data-add-cart="${product.id}">Thêm vào giỏ</button>
          <a class="button" href="cart.html">Đến giỏ hàng</a>
        </div>
      </div>
    `;
  ...
  function renderCart(cart) {
    ...
    $("#cartItems").innerHTML = items.map((item) => `
      <article class="list-item">
        <a class="product-image" href="product.html?id=${item.productId}">
          <img src="${imageUrl(item.imageUrl)}" alt="${html(item.productName)}">
        </a>
        <div>
          <h3><a href="product.html?id=${item.productId}">${html(item.productName)}</a></h3>
    ...
    `).join("") || `<div class="panel">Giỏ hàng đang trống. <a href="products.html">Mua hàng ngay</a></div>`;
  ...
  function initLogin() {
    ...
    $("#loginForm").addEventListener("submit", async (event) => {
      ...
      try {
        ...
        location.href = data.role === "ADMIN" || data.role === "ROLE_ADMIN" ? "admin.html" : "index.html";
      } catch (error) {
  ...
  async function initProfile() {
    ...
    $("#profileInfo").innerHTML = `
      ...
      <a class="button" href="orders.html">Xem đơn hàng</a>
    `;
